...
public class TileSourceBase {
	...
	protected static final String MBTILES = "mbtiles";
	...
	public String ID, BASEURL, NAME, IMAGE_FILENAMEENDING, GOOGLE_LANG_CODE, CACHE, MAPID, OVERLAYID, BLANKTILE;
	public int MAPTILE_SIZEPX, ZOOM_MINLEVEL, ZOOM_MAXLEVEL, ZOOM_MAXDNLD,
	URL_BUILDER_TYPE, // 0 - OSM, 1 - Google, 2 - Yandex, 3 - Yandex.Traffic, 4 - Google.Sattelite, 5 - openspace, 6 - microsoft, 8 - VFR Chart
	TILE_SOURCE_TYPE, // 0 - internet, 3 - MapNav file, 4 - TAR, 5 - sqlitedb, 6 - mbtiles
	YANDEX_TRAFFIC_ON,
	MAP_TYPE,
	...
	public TileSourceBase(Context ctx, String aId) throws SQLiteException, RException {
			...
			if(c != null) {
				if(c.moveToFirst()) {
					...
					if(c.getInt(2) == 1) { // Pair maps
						...
						this.BLANKTILE = json.optString(MixedMapsPreference.BLANKTILE, "123");
						if (this.BLANKTILE == "") this.BLANKTILE = null;
		...
		if (aId.contains(USERMAP_)) {
			...
			if (aId.toLowerCase(Locale.UK).endsWith(MBTILES)) {
				this.TILE_SOURCE_TYPE = 6;
				this.IMAGE_FILENAMEENDING = EMPTY;
			}
			else if (aId.toLowerCase(Locale.UK).endsWith(SQLITEDB)) {
		...
		if (BLANKTILE != null && !BLANKTILE.equals(""))
			blankTiles = BLANKTILE.split(",");
