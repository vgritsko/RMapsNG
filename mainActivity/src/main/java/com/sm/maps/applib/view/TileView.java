...
public class TileView extends View {
	...
	private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
		...
		public void onScaleEnd(ScaleGestureDetector detector) {
			...
			if (mTouchScale >= 1.5) {
				if (mTileSource.ZOOM_MAXLEVEL != zoom) {

					if (mTouchScale >= 6) zc = 3;
					else if (mTouchScale >= 3) zc = 2;
					else zc = 1;

					if (mTileSource.ZOOM_MAXLEVEL < (zoom + zc)) {
						zc -= zoom + zc - mTileSource.ZOOM_MAXLEVEL;
					}
				...
				if (mTileSource.ZOOM_MINLEVEL != getZoomLevel()) {
					...
					if (mTileSource.ZOOM_MINLEVEL > (zoom + zc)) {
						zc += zoom - zc - mTileSource.ZOOM_MINLEVEL;
					}
...
//	}
	...
	public double getTouchScale() { return mTouchScale; }

	public void setZoomLevel(final int zoom, final boolean resetMZoom) {

		int minz = 0;
		int maxz = 20;

		if(mTileSource != null) {
			minz = mTileSource.ZOOM_MINLEVEL;
			maxz = mTileSource.ZOOM_MAXLEVEL;
		}

		mZoom = Math.max(minz, Math.min(maxz, zoom));
	...
	public void zoomIn() {
		...
		if (getZoomLevel() == mTileSource.ZOOM_MAXLEVEL) {
			if (mTouchScale < 1)
				setZoomLevel(getZoomLevel(), true);
			return;
		}
	...
	public void zoomOut() {
		...
		if (getZoomLevel() == mTileSource.ZOOM_MINLEVEL) {
			if (mTouchScale > 1)
				setZoomLevel(getZoomLevel(), true);
			return;
		}
...
//		}
	...
	public void postQueuedInvalidate() {
		mQueue.add(new dummy_class());
		postInvalidate();
	}
